...
def proximity(subject_lat, subject_long, compare_lat, compare_long):
    distances = []
    for i in range(len(compare_lat)): # B
        distances.append(distance.distance((subject_lat, subject_long), (compare_lat[i], compare_long[i])).km)
...
def travel_time(subject_lat, subject_long, compare_lat, compare_long, api_key):
    ...
    routes = client.directions(((subject_lat, subject_long), (compare_lat, compare_long)))
